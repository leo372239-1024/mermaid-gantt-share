...
| `test/unit.js` | Node 单测（13 节 100 项）：解析器 + 数据完整性 + 序列化 round-trip + ics 生成器 + 分钟级时间口径 + 生成物一致性 + 过期快照体检 + **通道 C 闹钟口径（前后端同源校验）** + **编辑表单口径（类型/时间点/时刻清空）** + **合并写回护栏（页面过期不许删远端条目）**（`node test/unit.js`） |
| `tools/verify-ui.js` | 真实 Chromium 端到端自检（12 节 84 项）：**零第三方请求 / 本地字体已加载** / 红线标注左右位 / 分钟级标题 / 逐日斑马纹 / 日期数字居中 / 分钟级等比事件条 / ddl 面板 / Ctrl+S / 通道 C 闹钟清单 / **编辑表单（类型两选、时间点禁用结束时间、时刻可清空）** / **合并写回（用 GitHub API 替身跑一次真实保存，验证过期页面删不掉远端条目）** / 移动端竖横屏 + 出预览截图（开发期工具，需本机 playwright） |
...
> 第 N 周周一 = 9.7 + (N−1)×7 天。课程表**不进入待办提醒链路**
> （`deadlines.json` / `deadlines.ics` / 页面「🔔 提醒」/ 通道 C 闹钟）——周期性上课不是「截止待办」，
> 否则会出现「⏰ 截止：<课程名>」这类语义错误的日历事件。过滤口径 = section 名含「课程表」，
> 见 `tools/build-deadlines.js` 与 `js/viewer.js` 的 `isCourseTask()`（**两处必须同时改**）。
