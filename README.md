...
| `js/viewer.js` | 自研交互式 SVG 甘特渲染器：年月切换/回到今天/缩放（桌面 Ctrl+滚轮）/点击看详情/事件错色/智能防重叠标注/天刻度/双条今日红线/手机横屏整页 90° 旋转 |
...
| `tools/verify-ui.js` | 真实 Chromium 端到端自检：两条红线 / 分钟级标题 / ddl 面板 / Ctrl+S / .ics 通道 + 出预览截图（开发期工具，需本机 playwright） |
...
**今日线原理**：渲染时按浏览器当前日期绘制两条红色竖线 —— 实线为**当前时刻**（分钟级小数天偏移，页面上每分钟自动移动），虚线为**今日 24:00**（当日右边界）。全部本地计算，无服务器、无定时任务。
...
渲染特性：
- 时间轴：年份/月份行 + 逐日细网格；放大后【每一天】都有数字刻度（不再限奇数天，随缩放自动取舍）
- **今日两条红线**：实线 = **当前时刻**（分钟级，页面上每分钟自动移动），虚线 = **今日 24:00**（当日右边界，即次日 00:00）
- 配色：已完成统一灰；其余事件按序号错色（12 色调色板 × 步长 5，相邻事件不同色），关键节点（crit）叠加紫圈
- 标注：每条呈现「名称(起 至 止)」形态且**精确到分钟**，例如 `文艺汇演领票（9.10 14:00 至 9.10 17:00）`；起止同刻只显示一次（`9.10 18:45`），无时刻的条目保持纯日期（`9.13`）；超长注记自动降级、条内放不下时条尾外置并强制显示；条/点旁标注按"右/左/上/下"四档试位 + 双向碰撞检测防重叠
